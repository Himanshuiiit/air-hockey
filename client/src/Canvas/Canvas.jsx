...
const Canvas = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    canvas.onmousemove = (event) => {
      ...
      socket.emit("mouse-position", mouse);
      var now = Date.now();
      const currentmY = event.screenY;
      const currentmX = event.screenX;
    ...
    socket.on("location", (mousePos) => {
      striker2.x = canvas.width - mousePos.x;
      striker2.y = canvas.height - mousePos.y;
    });
